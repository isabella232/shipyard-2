...
def build_application(request):
    ...
    configuration = form.data.get('configuration')
    hosts = form.data.getlist('hosts')
        
    archive_name = request.FILES.get('application').name
    application_name = archive_name.split('.')[0]
    handle_upload(interpreter, request.FILES.get('application'), archive_name, application_name)

    args = (interpreter, version, os, port, application_name, launch, after_launch, configuration)
